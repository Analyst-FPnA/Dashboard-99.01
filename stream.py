...
with col[1]:
    ...
    bulan = st.multiselect("BULAN:", list_bulan, default = ['September','October','November'], on_change=reset_button_state)
...
if st.session_state.button_clicked:
        ...
        if cab != 'All' :
            df_vendor = df_test[(df_test['Nama Cabang']==cab)&(df_test['PIC'].isin(df_test['PIC'].unique() if pic==['All'] else pic))].groupby(['Month','Pemasok','Kode #']).agg({'#Prime.Qty': 'sum','#Purch.Total': 'sum'}).reset_index()
            ...
            df_vendor = df_test[(df_test['PIC'].isin(df_test['PIC'].unique() if pic==['All'] else pic))].groupby(['Month','Pemasok','Kode #']).agg({'#Prime.Qty': 'sum','#Purch.Total': 'sum'}).reset_index()   
...
if ('filtered_df_test' in st.session_state) :
    ...
    df_test = df_vendor.pivot(index=['Pemasok','Kode #','Nama Barang'],columns='Month',values=wa_qty).fillna('').reset_index()
    ...
    df_test = df_test.replace('',0).style.format(lambda x: '' if x==0 else x).background_gradient(cmap='Reds', axis=1, subset=df_test.columns[3:-1])
    st.dataframe(df_test, use_container_width=True, hide_index=True)
