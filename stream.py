...
if len(bulan)>=3:
    df_test[f'Diff {bulan[-3]} - {bulan[-2]}'] = df_test.applymap(lambda row: 0 if ((row[bulan[-2]] == '') or (row[bulan[-3]]=='')) else ((row[bulan[-2]] - row[bulan[-3]]) / row[bulan[-3]]), axis=1)
    df_test[f'Diff {bulan[-2]} - {bulan[-1]}'] = df_test.applymap(lambda row: 0 if ((row[bulan[-1]] == '') or (row[bulan[-2]]=='')) else ((row[bulan[-1]] - row[bulan[-2]]) / row[bulan[-2]]), axis=1)
