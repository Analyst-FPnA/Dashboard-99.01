...
if st.session_state.button_clicked:
    ...
    df_test.loc[:,[x for x in df_test.columns if x in list_bulan]] = df_test.loc[:,[x for x in df_test.columns if x in list_bulan]].applymap(lambda x: f'{x:.2f}' if isinstance(x, float))
    st.write(df_test2)
    st.write(df_test)
