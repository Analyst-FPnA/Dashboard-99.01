...
if ('filtered_df_test' in st.session_state) :
    ...
    df_test = df_test.replace('',np.nan).style.applymap(lambda x: 'background-color: white' if pd.isna(x) else '',
                              subset=df_test.columns[2:-1]).background_gradient(cmap='Reds', axis=1, subset=df_test.columns[2:-1])
